...
extern CString strPort1name;
extern CString strPort2name;
extern CString strPort3name;
...
BOOL CCommThread::OpenPort(CString strPortName, 
	...
	// Local 변수.
		...
		//--> 포트 열기에 실행하면..
		if (m_hComm == (HANDLE) -1) {
			AfxMessageBox("Fail in Opening Serial Port.");
			return FALSE;
		}	
...
#define N_UART_READING  30
DWORD	ThreadWatchComm(CCommThread* pComm) {
   DWORD           dwEvent;
   OVERLAPPED      os;
   BOOL            bOk = TRUE;
   BYTE            buff[N_UART_READING];      // 읽기 버퍼
   ...
   while (pComm ->m_bConnected) { 
        ...
        if ((dwEvent & EV_RXCHAR) == EV_RXCHAR) {
            // 포트에서 읽을 수 있는 만큼 읽는다.
				//--> buff 에 받아놓고..
			do {
                dwRead = pComm->ReadComm(buff, N_UART_READING);
            ...
            if (pComm->m_sPortName == strPort1name) {
                BOOL ret = ::PostMessage(hCommWnd, WM_COMM_READ1, 0, 0);//CSerialComDlg로 데이터가 들어왔다는 메시지를 보냄
                if (!ret) {
                    TRACE("PostMessage 실패\n");
                }
            }
			else if (pComm->m_sPortName == strPort2name){
				BOOL ret = ::PostMessage(hCommWnd, WM_COMM_READ2, 0, 0);//CSerialComDlg로 데이터가 들어왔다는 메시지를 보냄	
                if (!ret) {
                    TRACE("PostMessage 실패\n");
                }
            }
			else if (pComm->m_sPortName == strPort3name) {
				BOOL ret = ::PostMessage(hCommWnd, WM_COMM_READ3, 0, 0);//CSerialComDlg로 데이터가 들어왔다는 메시지를 보냄	
				if (!ret) {
					TRACE("PostMessage 실패\n");
				}
			}
